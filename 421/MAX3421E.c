...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	//write val via SPI
	//make array of val and reg
	BYTE POTARAFUSION [2] = {reg + 2, val};
	int VEGITO;
	VEGITO = alt_avalon_spi_command(SPI0_BASE, 0, 2, POTARAFUSION, 0, NULL,0);
	if (VEGITO < 0){
		printf("error_u_fcked_up/n");
		return;
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode: ur psuedo code trash bruv
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	BYTE HMNCENTIPEDE [nbytes + 1];
	HMNCENTIPEDE[0] = reg+2;
	for(int i = 1; i < nbytes + 1; i++){
		HMNCENTIPEDE[i] = data[i-1];
	}
		int VEGITO;
		VEGITO = alt_avalon_spi_command(SPI0_BASE, 0, nbytes + 1, HMNCENTIPEDE, 0, NULL,0);
		if(VEGITO < 0) {printf("error/n");}
		return (data + nbytes);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	int VEGITO;
	BYTE KRILLIN;
	VEGITO = alt_avalon_spi_command(SPI0_BASE, 0, 1, &reg, 1, &KRILLIN,0);
	if(VEGITO < 0) {printf("error/n");}
	return KRILLIN;
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int VEGITO;
	VEGITO = alt_avalon_spi_command(SPI0_BASE, 0, 1, &reg, nbytes, data,0);
	if(VEGITO < 0) {printf("error/n");}
	return (data + nbytes);
